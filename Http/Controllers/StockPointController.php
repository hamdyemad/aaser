...
class StockPointController extends Controller
{
    public function add(StockPointRequest $request)
    {
        ...
        try
        {
            ...
            $service_provider = ServiceProvider::create([
                'name' => 'مقدم خدمة ' . $stock_point->name,
                'email' => $randomEmail,
    ...
    public function addServiceRequest(AddServiceRequestRequest $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'appointment' => ['array', 'required'],
            'appointment.*' => ['required', 'date', 'date_format:Y-m-d'],
        ]);
        ...
        $validator->after(function ($validator) use ($request) {
            ...
            foreach ($serviceIds as $index => $serviceId) {
                ...
                if($serviceStockPoint_end_date->lt($inserted_date)) {
                    $validator->errors()->add("date", "the date inserted less than the service date");
                }
