    ...
    <div class="receipt">
        ...
        <table class="table" cellspacing="0">
            ...
            @php
                ...
                $total = 0;
            @endphp
            @foreach ($reward_request->requestStockPoint as $requestStockPoint)
                @php
                    $points += $requestStockPoint['service']['point'];
                    $total += $requestStockPoint['products_count'] * $requestStockPoint['service']['amount'];
                @endphp
                <tr class="table-row">
                    <td>{{ $requestStockPoint['service']['name'] }}</td>
                    <td>{{ $requestStockPoint['service']['amount'] }}</td>
                    <td>{{ $requestStockPoint['products_count'] }}</td>
                    <td>{{ $requestStockPoint['products_count'] * $requestStockPoint['service']['amount'] }}</td>
                    <td>{{ $requestStockPoint['service']['appointment'] }}</td>
                    <td>{{ $requestStockPoint['service']['point'] }}</td>
                </tr>
            @endforeach
        </table>
