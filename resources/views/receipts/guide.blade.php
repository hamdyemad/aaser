    ...
    <div class="receipt">
        ...
        <div class="info-section">
            <div class="user-info">
                ...
                <p>النقاط: {{ $reward_request->user->points }}</p>
            </div>
            <div class="date-info">
        ...
        <table class="table" cellspacing="0">
            ...
            @php
                $points = 0;
            @endphp
            @foreach ($reward_request->requestGuide as $requestGuideOffer)
                @php
                    $points += $requestGuideOffer['offer']['points'];
                @endphp
                <tr class="table-row">
                    <td>{{ $requestGuideOffer['offer']['name'] }}</td>
                    <td>{{ $requestGuideOffer['offer']['discount'] . '%' }}</td>
                    <td>{{ $requestGuideOffer['offer']['points'] }}</td>
                </tr>
            @endforeach
        </table>
        ...
        {{-- <div class="footer">
            <div>
                ...
                <p>صالحة للاستخدام مره واحدة فقط</p>
                <p>غير قابله للاسترداد او التحويل</p>
                <p>يجب عرض القسيمة قبل طلب الحساب</p>
                <p>لا يمكن العرض بين هذا العرض او اي عروض اخرى</p>
            </div>
        </div> --}}
    </div>
</body>
