    ...
    <div class="receipt">
        ...
        <table class="table" cellspacing="0">
            ...
            @php
                ...
                $total = 0;
            @endphp
            @foreach ($reward_request->requestActivity as $requestActivity)
                @php
                    $points += $requestActivity['serviceActivity']['earn_points'];
                    $total += $requestActivity['qty'] * $requestActivity['serviceActivity']['amount'];
                @endphp
                <tr class="table-row">
                    <td>{{ $requestActivity['serviceActivity']['service_type'] }}</td>
                    <td>{{ $requestActivity['serviceActivity']['amount'] }}</td>
                    <td>{{ $requestActivity['qty'] }}</td>
                    <td>{{ $requestActivity['qty'] * $requestActivity['serviceActivity']['amount'] }}</td>
                    <td>{{ $requestActivity['date'] }}</td>
                    <td>{{ $requestActivity['serviceActivity']['earn_points'] }}</td>
                </tr>
            @endforeach
        </table>
